...
//
// 架构职责：
// - ViewModel 负责修改当前状态（通过 setLoading/setSuccess/setEmpty/setError/setIdle）
// - StateView 负责根据状态切换 UI（loading/error/empty/idle/success）
// - 业务页面不需要关心状态切换逻辑，只需要在 builder 里写正常内容即可
//
// 状态流转说明：
// idle → loading → success / empty / error
// 任何状态都可以通过 setIdle 回到初始状态
// loading 状态下一般不会再次触发 loading（由 BaseViewModel 的 asyncRequest 防抖保证）
//
// 使用示例：
// ```dart
// class MyViewModel extends BaseViewModel {
//   Future<void> loadData() async {
//     await asyncRequest(() => repository.fetchData());
//     // asyncRequest 内部会自动调用 setLoading/setSuccess/setEmpty/setError
//   }
// }
// ```
enum ViewState {
