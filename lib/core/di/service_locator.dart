...
//
// 注册顺序：
// 先注册底层依赖（ApiService），再注册上层依赖（Repository、ViewModel），
// 因为上层依赖需要从容器中获取底层依赖。
//
// 使用方式：
// ```dart
...
///
/// 防重复注册：如果 ApiService 已经注册（热重启或多次调用），直接返回。
///
/// 注册顺序：
/// 1. ApiService（底层网络服务）
...
Future<void> setupServiceLocator() async {
  // ---- 防重复注册 ----
  // 测试或热重启场景下可能多次调用 setupServiceLocator
  // 检查 ApiService 是否已注册来判断是否重复调用
